...
function autenticar(req, res) {
  ...
  if (email == undefined) {
    ...
    usuarioModel
      .autenticar(email, cifraDeCesar(senha, 3))
      .then((resposta) => {
        if (resposta.length == 1) {
          validarLogin(resposta, res);
        } else {
          usuarioModel.autenticar(email, senha).then((resposta) => {
            if (resposta.length == 1) {
              validarLogin(resposta, res);
            } else {
    ...
    usuarioModel
      ...
      .then((resultado) => {
        if (resultado.length > 0) {
          ...
          res
            .status(400)
            .send("Erro não foi possível encontrar usuário com esse id!");
        }
      })
      .catch((erro) => {
        console.log(erro);
...
const alertaUsuario = (req, res) => {
  const idUsuario = req.params.idUsuario;
  const startTime = Date.now();

  usuarioModel.verificarAlerta(idUsuario)
    .then((resp) => {
      const endTime = Date.now();
      console.log(`Tempo de resposta da consulta: ${endTime - startTime} ms`);
...
const carregarMaisAlertas = (req, res) => {
  ...
  usuarioModel.carregarMaisAlertas(idUsuario, ultimoId)
    .then((resp) => {
      const endTime = Date.now();
      console.log(`Tempo de resposta da consulta: ${endTime - startTime} ms`); 
      return res.json(resp);
    ...
    .catch((erro) => {
      console.error("Erro ao carregar mais alertas:", erro);
      res.status(500).json(erro.sqlMessage);
    });
};


const updateRegistroAlerta = (req, res) => {
  const { idCaptura, idAlerta } = req.body;

  if (!idCaptura || !idAlerta) {
    return res.status(400).json({ error: "Os campos idCaptura e idAlerta são obrigatórios." });
  }

  usuarioModel.updateAlertaVisualizado(idCaptura, idAlerta)
    .then(() => {
      return res.status(200).json({ message: "Registro de alerta atualizado com sucesso." });
    })
    .catch((error) => {
